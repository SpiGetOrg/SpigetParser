...
public class ResourcePageParser {
    ...
    public Resource parse(@NotNull Document document, @NotNull ListedResource base) {
        ...
        // Links
        {
            ...
            {// Documentation
                Element resourceTabExtra = document.select("li.resourceTabExtra").first();
                if (resourceTabExtra != null) {
                    Element extraLink = resourceTabExtra.select("a").first();
                    String extraLinkHref = extraLink == null ? null : extraLink.attr("href");
                    if (extraLinkHref != null && extraLinkHref.endsWith("documentation")) {
                        resource.getLinks().put("documentation", extraLinkHref);
                    }
                }
            }
